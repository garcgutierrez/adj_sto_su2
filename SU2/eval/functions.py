...
def function( func_name, config, state=None ):
    ...
    # print('ff:{}'.format(state['FUNCTIONS']))
...
def aerodynamics( config, state=None ):
    ...
    notFirst=True
    try:
        sto = state.getSubfolder()
        if('M_0' in sto):
            notFirst = False
        state.deleteSubfolder()
        # print('sto:{}'.format(sto+'DIRECT'))
    except:
        sto = False
    ...
    if(sto==False):
        ...
        #print('link list:{}'.format(link))
        with redirect_folder( sto+'DIRECT', pull, link ) as push:
            #print('redirecting')
            if(notFirst==False):
                1+1#print('lul')
            else:
                try:
                    ...
                    # print(os.listdir())
                    os.symlink(pwd+"/../../M_0/mesh_NACA0012_inv_deform.su2",pwd+"/mesh_NACA0012_inv_deform.su2")
...
def geometry( func_name, config, state=None ):
    ...
    if(sto==False):
        ...
        if not geometry_done:    
            ...
            # output redirection
            with redirect_folder(sto + 'GEOMETRY', pull, link ) as push:
                with redirect_output(log_geom):     
...
def update_mesh(config,state=None):
    ...
    if(sto==True):
        if deform_set and deform_todo:
            ...
            # output redirection
            with redirect_folder(sto+'DEFORM',pull,link) as push:
                with redirect_output(log_deform):
