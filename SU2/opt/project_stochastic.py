...
class Project_Stochastic(object):
    """ project = SU2.opt.Project(self,config,state=None, 
                                  designs=[],folder='.', Machs = [0.01])
    ...
    def __init__( self, config,state=None ,UQ=None, 
                  designs=None, folder='.' ,
                  warn = True                ):
        self.UQ = UQ
    ...
    def _eval(self,config,func,*args):
        ...
        #for i in np.arange(self.UQ.Nquadrature):
        ...
        with redirect_folder(folder,pull,link,force=False) as push:
            ...
            # check for update
            if design.state[i].toc(timestamp):
    ...
    def obj_df(self,dvs):
        func = su2eval.obj_dfs
        konfig,dvs = self.unpack_dvs(dvs)
        return self._eval(konfig, func,dvs)

    def con_ceq(self,dvs):
        func = su2eval.con_ceqs
        konfig,dvs = self.unpack_dvs(dvs)
        return self._eval(konfig, func,dvs)
    
    def con_dceq(self,dvs):
        func = su2eval.con_dceqs
        konfig,dvs = self.unpack_dvs(dvs)
        return self._eval(konfig, func,dvs)
    
    def con_cieq(self,dvs):
        func = su2eval.con_cieqs
        konfig,dvs = self.unpack_dvs(dvs)
        return self._eval(konfig, func,dvs)
    
    def con_dcieq(self,dvs):
        func = su2eval.con_dcieqs
        konfig,dvs = self.unpack_dvs(dvs)
        return self._eval(konfig, func,dvs)
    ...
    def init_design(self,config,closest=None):
        ...
        ztates = [copy.deepcopy(ztate) for i in np.arange(self.UQ.Nquadrature)]
        # start new design (pulls files to folder)
        design = su2eval.Design_Stochastic(konfig,ztates,folder,self.UQ)
    ...
    def compile_results(self,default=np.nan):
        ...
        for i,design in enumerate(self.designs):
            for jj in np.arange(self.UQ.Nquadrature):
                for key in design.state[jj].FUNCTIONS.keys():
                    results.FUNCTIONS[key] = []
                    for key in design.state[jj].GRADIENTS.keys():
                        results.GRADIENTS[key] = []
                    for TYPE in design.state[jj].HISTORY.keys():
                        if not TYPE in results.HISTORY:
                            results.HISTORY[TYPE] = su2util.ordered_bunch()
                    for key in design.state[jj].HISTORY[TYPE].keys():
                        results.HISTORY[TYPE][key] = []
            this_ndv = len( design.state[0].design_vector() )
        ...
        for design in self.designs:
            ...
            this_designvector = design.state[0].design_vector()
            results.VARIABLES.append( this_designvector )
            for key in results.FUNCTIONS.keys():
                func_array = []
                for jj in np.arange(self.UQ.Nquadrature):
                    if key in design.state[jj].FUNCTIONS:
                        func_array.append(design.state[jj].FUNCTIONS[key])
                    else:
                        func_array.append(default)

                new_func = self.UQ.computeProperties(func_array)[0]
                # print('new_func:{}'.format(new_func))
                # print('func_array:{}'.format(design.state[0].FUNCTIONS[key]))
                results.FUNCTIONS[key].append(new_func)
            for key in results.GRADIENTS.keys():
                fun_grad = []
                for jj in np.arange(self.UQ.Nquadrature):
                    if key in design.state[jj].GRADIENTS:
                        fun_grad.append(design.state[jj].GRADIENTS[key])
                    else:
                        fun_grad.append([default] * len( this_designvector ))
                new_grad = self.UQ.computeProperties(fun_grad)[0]
                ...
                for key in results.HISTORY[TYPE].keys():
                    ...
                    for jj in np.arange(self.UQ.Nquadrature):
                        if key in results.FUNCTIONS.keys():
                            func_array.append(results.FUNCTIONS[key][-1])
                        elif ( TYPE in design.state[jj].HISTORY.keys() and
                               key in design.state[jj].HISTORY[TYPE].keys() ):
                            func_array.append(design.state[0].HISTORY[TYPE][key][-1])
                        else:
                            func_array.append(default)
                    new_func = self.UQ.computeProperties(func_array)[0]
